...
public class Spliter {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0;i < lastfilenum;i ++){
            ...
            bw.write("execute if score $dream_finalskill_psummon_timer dream_timer matches "+cs.get(index).time+".."+cs.get(index+each>cs.size()?cs.size()-1:index+each-1).time+" run function dream:memory_story/final_boss1/timerzip/0-"+(i+1)+"\n");
            index += each;
        }
        ...
        for(int i = 0;i < lastfilenum;i ++){
            ...
            bw.write("execute if score $dream_finalskill_psummon_timer dream_timer matches "+cs.get(index).time+".."+cs.get(index+each*each>cs.size()?cs.size()-1:index+each*each-1).time+" run function dream:memory_story/final_boss1/timerzip/1-"+(i+1)+"\n");
            index += each*each;
        }
        ...
        for(int i = 0;i < lastfilenum;i ++){
            ...
            bw.write("execute if score $dream_finalskill_psummon_timer dream_timer matches "+cs.get(index).time+".."+cs.get(index+each*each*each>cs.size()?cs.size()-1:index+each*each*each-1).time+" run function dream:memory_story/final_boss1/timerzip/2-"+(i+1)+"\n");
        ...
        for(int i = 0;i < lastfilenum;i ++){
            ...
            bw.write("execute if score $dream_finalskill_psummon_timer dream_timer matches "+cs.get(index).time+".."+cs.get(index+each*each*each*each>cs.size()?cs.size()-1:index+each*each*each*each-1).time+" run function dream:memory_story/final_boss1/timerzip/3-"+(i+1)+"\n");

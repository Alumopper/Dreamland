...
execute if entity @e[tag=dream_found] run tellraw @s [{"translate":"dream.info.locate.sf"},{"nbt":"Pos","entity": "@e[tag=dream_found,limit=1]","color": "green","clickEvent": {"action": "suggest_command","value": "/tp @s @e[tag=dream_sf,limit=1,sort=nearest]"}}]

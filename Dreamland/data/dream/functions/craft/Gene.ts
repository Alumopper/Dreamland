...
fs.readFile(file, 'utf8', (err, data) => {
    ...
    //遍历配方树，生成所有可能的合成路径
    //execute if score @s dream_counter matches 0 if data storage dream:crafting_table {id:"dreamland:dreamdust"} run function dream:craft/tree/dreamdust 
...
function walkTree(recipe:RecipeTree, filePath : string){
    ...
    for(let child of recipe.children){
        ...
        //execute if score @s dream_counter matches 0 if data storage dream:crafting_table {id:"dreamland:dreamdust"} run function dream:craft/tree/dreamdust 
        let command = "execute if score @s dream_counter matches 0 if data storage dream:crafting_table {id:\"" + child.item + "\"} run function dream:craft/" + path.relative(__dirname,filePath).replace('\\','/') + "/" + recipe.getFunctionName() + "/" + child.getFunctionName();

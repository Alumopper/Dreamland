...
execute if score rand_output math_output >= dream_temp_inf_ech_enrich dream_counter as @e[nbt={Item:{tag:{tag:{isTemp:1b}}}}] at @s run loot spawn ~ ~ ~ loot dream:others/flint
...
#手持云伞让玩家缓降
execute if entity @s[tag=!dream_effect_levitation255] if data storage dream:player_data {SelectedItem:{tag:{id:"dreamland:cloud_umbrella"}}} run tag @s add dream_effect_levitation255
execute if entity @s[tag=!dream_effect_levitation255] if data storage dream:player_data {SelectedItem2:{tag:{id:"dreamland:cloud_umbrella"}}} run tag @s add dream_effect_levitation255 
#手持法杖或梦境护符显示魔法值
...
execute if data entity @s {Inventory:[{Slot: 100b,tag: {id: "dreamland:dream_god_boots"}}, {Slot: 101b,tag: {id: "dreamland:dream_god_leggings"}}, {Slot: 102b,tag: {id: "dreamland:dream_god_chestplate"}},{Slot: 103b,tag: {id: "dreamland:dream_god_helmet"}}]} run advancement grant @s only dream:beta/dream_god

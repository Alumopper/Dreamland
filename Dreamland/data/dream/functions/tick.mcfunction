#合成梦境工作台
execute as @e[type=item,nbt={Item:{id:"minecraft:crafting_table",Count:1b}}] at @s if entity @e[type=item,distance=..1,nbt={Item:{id:"minecraft:glowstone_dust",Count:1b,tag:{dreamland:{id:"dreamland:dreamdust"}}}}] run function dream:craft/crafting_table
#方块挖掘//临时
execute as @e[tag=dream_block] at @s unless block ~ ~ ~ barrel run function dream:break
#工作台合成
execute as @e[tag=dream_craftingtable] unless entity @s[tag=dream_crafting] at @s if entity @a[distance=0..4] unless block ~ ~ ~ minecraft:barrel{Items:[{Slot:9b,tag:{dreamland:{id:"dreamland:magic_plate"}}}]} run function dream:craft/craft
execute as @e[tag=dream_craftingtable] unless entity @s[tag=dream_crafting] at @s if entity @a[distance=0..4] if data storage dream:crafting_table {Items:[{Slot:9b,tag:{dreamland:{id:"dreamland:magic_plate"}}}]} run function dream:craft/magic_craft
...
#判断梦之尘是否在床上
execute as @e[type=item,nbt={Item:{tag:{dreamland:{id:"dreamland:dreamdust"}}}}] at @s if block ~ ~ ~ #minecraft:beds align xyz positioned ~0.5 ~ ~0.5 run function dream:sleep/bed
#去除床的标记

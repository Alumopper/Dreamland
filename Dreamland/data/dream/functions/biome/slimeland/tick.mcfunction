...
#挖掘生成史莱姆
execute as @a[predicate=dream:if_entity/in_slimeland] if score @s dream_dig_sb matches 1.. if predicate dream:1in10 at @s as @e[type=item,nbt={Item:{id:"minecraft:slime_block"}},distance=0..4] at @s run function dream:biome/slimeland/summon_slime
#气球史莱姆的数量控制
scoreboard players set dream_biome_slimeland_tick_temp dream_counter 0
execute store result score dream_biome_slimeland_tick_temp dream_counter if entity @e[tag=entity.dream.balloon_slime]

#boss血量显示
execute store result bossbar dream:boss value run data get entity @e[tag=entity.dream.dark_illusioner,limit=1] Health
...
execute at @e[tag=entity.dream.dark_illusioner] if predicate dream:1in100 run summon vex ~ ~ ~
#距离限制
execute as @a at @s unless entity @e[tag=entity.dream.dark_illusioner,distance=..10] run effect give @s poison 1 1
#检测boss是否存活
execute unless entity @e[tag=entity.dream.dark_illusioner] run function dream:entity/dark_illusioner/end
#如果玩家全挂了

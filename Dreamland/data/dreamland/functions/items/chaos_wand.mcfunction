...
#生成魔法球
summon area_effect_cloud ^ ^ ^0.09 {Tags:[entity.dream.magicball.chaos,dream_just,entity.dream.magicball],Duration:200000,Radius:0.0f}
scoreboard players set @e[tag=entity.dream.magicball.chaos,tag=dream_just] dream_magicball_energy 150
#其他注魔
execute if data storage dreamland:player_data {SelectedItem:{tag:{dreamland:{infusion:1b}}}} run tag @e[tag=entity.dream.magicball.chaos,tag=dream_just] add entity.dream.magicball.speed
execute if data storage dreamland:player_data {SelectedItem:{tag:{dreamland:{infusion:2b}}}} run tag @e[tag=entity.dream.magicball.chaos,tag=dream_just] add entity.dream.magicball.poison
#水晶

...
execute if data storage dreamland:player_data {SelectedItem:{tag:{dreamland:{infusion:1b}}}} run tag @e[tag=dream_just] add entity.dream.magicball.speed
execute if data storage dreamland:player_data {SelectedItem:{tag:{dreamland:{infusion:2b}}}} run tag @e[tag=dream_just] add entity.dream.magicball.poison

...
execute as @e[tag=dream_dm,limit=1,sort=nearest,distance=0..] run tag @s add dream_found
...
execute if entity @e[tag=dream_found] run tellraw @s [{"translate":"info.dreamland.locate.dm"},{"nbt":"Pos","entity": "@e[tag=dream_found,limit=1]","color": "green","clickEvent": {"action": "suggest_command","value": "/tp @s @e[tag=dream_dm,limit=1,sort=nearest]"}}]

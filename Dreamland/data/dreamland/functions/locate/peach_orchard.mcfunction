...
execute if entity @e[tag=dream_found] run tellraw @s [{"translate":"info.dreamland.locate.po"},{"nbt":"Pos","entity": "@e[tag=dream_found,limit=1]","color": "green","clickEvent": {"action": "suggest_command","value": "/tp @s @e[tag=dream_po,limit=1,sort=nearest]"}}]

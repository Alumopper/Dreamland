...
execute as @e[tag=block.dream.dream_portal,limit=1,sort=nearest,distance=0..1.5] if entity @s[tag=block.dream.dream_portal_sweetdream] at @a[tag=dream_will_tp] in dreamland:sweetdream run tp @a[tag=dream_will_tp] ~ ~ ~
execute as @e[tag=block.dream.dream_portal,limit=1,sort=nearest,distance=0..1.5] if entity @s[tag=block.dream.dream_portal_nightmare] at @a[tag=dream_will_tp] in dreamland:nightmare run tp @a[tag=dream_will_tp] ~ ~ ~
tag @s remove dream_will_tp
execute at @s run summon marker ~ ~ ~ {Tags:["block.dream.dream_portal_pointer"]}
#如果第一次进入，启用崩毁倒计时

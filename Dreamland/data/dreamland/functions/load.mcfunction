...
scoreboard objectives add dream_timer dummy "计时器"
    schedule function dreamland:loadinf 20t append
...
#bossbar区
#经验池，全部玩家通用
bossbar add dreamland:xp ["经验池   ",{"score": {"objective": "dream_xp","name": "xp"}},"/10000"]
bossbar set dreamland:xp color green
bossbar set dreamland:xp max 10000
#boss专用
bossbar add dreamland:boss ""
#final_boss的护盾
bossbar add dreamland:boss_sheild ""

#强制加载
execute in dreamland:chaos run forceload add -10 -10 10 10

#结构区
#黑洞生成
execute unless score dream_is_blackhole_generated dream_counter matches -2147483648..2147483647 run function dreamland:structure/black_hole/load

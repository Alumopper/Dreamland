...
#进入梦境
execute as @a if score @s dream_tpwait matches 0.. run scoreboard players add @s dream_tpwait 1
...
execute as @a if score @s dream_sleeptime matches 11800 run tellraw @s [{"text": "梦快醒了......","color": "gray","italic": true}]
execute as @a if score @s dream_sleeptime matches 12000.. run function dream:sleep/leave
scoreboard players set @a[predicate=!dream:if_entity/in_dream] dream_sleeptime -1
#梦境塔生成计时
...
#阶段二：200分钟，一阶段结束后玩家再次利用梦境传送门进入梦境时开始。这一时间段内玩家不能够再使用梦境传送门，并且在200分钟后会被强制传送到终末指梦边缘。
